...
def your_ik(robot, new_pose : list or tuple or np.ndarray, 
    ...
    # delta_x = new_pose - pose
    # delta_x = pose_7d_to_6d(delta_x)
    # print(delta_x)

    # delta x = x y z roll pitch yaw (use noap to confirm the orientation of the angle)
    ...
    while((iters<=max_iters)):
        ...
        iters += 1
    '''for i in range(7):
        if (new_tmp_q[i]<joint_limits[i][0] and new_tmp_q[i]>joint_limits[i][1]):
            tmp_q[i] = new_tmp_q[i]'''
    # print(tmp_q)
