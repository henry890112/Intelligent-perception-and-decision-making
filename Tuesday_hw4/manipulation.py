...
def get_cam_params(info_dict : dict, cam_param : dict):
    ...
    view_matrix = p.computeViewMatrix(    # 在line 474 cam_coordinate中設計的
...
def randomize_standing_pose(base_pos, base_rot):  # 每次物體擺放的位置皆不同
...
def create_pcd_from_rgbd(rgb : np.ndarray, depth : np.ndarray, 
    ...
    x_ratio = (ix.ravel() - cx) / fx  # np可同時對全部數值做操作
    y_ratio = (iy.ravel() - cy) / fy

    z = depth.ravel() / dscale  # 和flatten功能相似但不會產生副本(不會浪費記憶體空間)
    x = z * x_ratio
    y = z * y_ratio
    ...
    y = y[cond]
    z = z[cond]
    # point 和 color的list shape皆為(n, 3)
    points = np.vstack((x, y, z)).T  #　(3, n)的array變成(n, 3)
    colors = np.reshape(rgb,(depth.shape[0] * depth.shape[1], 3)) /255.
    colors = colors[cond]

    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(np.array(points))  # point 跟 color皆為(n, 3)的np array
    pcd.colors = o3d.utility.Vector3dVector(np.array(colors))
    pcd.transform(extr)  # 經由extrinsic轉換後的pcd (from camera pose to world coordinate)
...
def load_kpts_from_depth(intrinsic : list or np.ndarray, keypoints : list or np.ndarray, depth_path : str):
    ...
    kpts_3d =  np.vstack((x, y, z))   # [[x1, x2, ......., xn]
                                      #  [y1, y2, ......., yn]
    return kpts_3d    # (3, n)        #  [z1, z2, ......., zn]]  # 此些點皆在camera coordinate下也就是真實的xyz
...
def get_src2dst_transform_from_kpts(src_kpts_homo : np.ndarray, src_extrinsic : np.ndarray, 
    ...
    src_kpts_homo = (src_extrinsic @ src_kpts_homo)[:3,:]  # 將camera pose 的keypoint轉換到world frame
    dst_kpts_homo = (dst_extrinsic @ dst_kpts_homo)[:3,:]  # 輸出(3, n)
    
    src_mean = np.mean(src_kpts_homo, axis=1).reshape((3, 1)) # 對每個列(axis = 1)求均值 outpur = (3, 1)
    dst_mean = np.mean(dst_kpts_homo, axis=1).reshape((3, 1))

    src_points_ = src_kpts_homo - src_mean  #(3, n)
    dst_points_ = dst_kpts_homo - dst_mean  #(3, n)

    W = dst_points_ @ src_points_.T  # W = 3*3
    
    u, s, vh = np.linalg.svd(W, full_matrices=True)  # 做svd分解求出U, V矩陣  SVD是用來求出兩組xyz的轉移矩陣
    R = u @ vh
...
def main(args):
    ...
    p.configureDebugVisualizer(p.COV_ENABLE_GUI, 0)  # 1是讓左邊欄位會顯示
    p.resetDebugVisualizerCamera(   # 調整一進去的角度
        cameraDistance=0.5,
        cameraYaw=90,
        cameraPitch=0,
        # cameraPitch=-30,
    ...
    global state   # 宣告global代表別的func也可使用此參數的值...
    ...
    cam_coordinate = {
        ...
        'cameraTargetPosition': [0.35, 0.1, 0.76],  # 往-y的方向看去
        'cameraUpVector': [0.0, 0.0, 1.0],
    }
    cam_params = get_cam_params(info_dict, cam_coordinate)  # 得到相機的參數
    ...
    while repeat:
        ...
        # 產生的皆為真實的xyz(已經經由intrinsic轉換過)，真實的XYZ在經由extrinsix就會變成轉換座標過後的真實XYZ
        template_3d_kpts = load_kpts_from_depth(intrinsic, template_3d_kpts, template_depth_path) # (3, n)
        ...
        num_kpts = template_3d_kpts.shape[1]  # (3, n)  n為keypoint的個數
        template_3d_kpts_homo = np.vstack((template_3d_kpts, np.full((num_kpts,), 1.0))) # 變(4, n)
