...
def collision(x1, y1, x2, y2):
    ...
    for i in range(len(x)):
        ...
        color.append(img[int(y[i]), int(x[i])])  # image color index is (y, x)
    if (0 in color):
        return True #collision
    else:
        return False #no-collision

# check the  collision with obstacle and trim and then decide the orientation of the stepSide
...
def RRT(img, img2, start, end, stepSize):
    ...
    while pathFound==False:
        ...
        if directCon and nodeCon:
            ...
            cv2.line(img2, (int(tx),int(ty)), (int(node_list[nearest_ind].x),int(node_list[nearest_ind].y)), (0,255,0), thickness=1, lineType=8) #green line
            cv2.line(img2, (int(tx),int(ty)), (end[0],end[1]), (255,0,0), thickness=2, lineType=8) # blue line
            ...
            cv2.circle(img2, (int(tx), int(ty)), 2, (0,0,255), thickness=3, lineType=8)   
            cv2.line(img2, (int(tx), int(ty)), (int(node_list[nearest_ind].x), int(node_list[nearest_ind].y)), (0,255,0), thickness=1, lineType=8)  # green line
            cv2.imwrite("media/"+str(i)+".jpg",img2)  # 將所有點的照片存下來
            cv2.imshow("sdc",img2)
            cv2.waitKey(1)
            continue
    ...
    node_list[i].parent_x.append(end[0])  # target的點
    node_list[i].parent_y.append(end[1])  # target的點
    
    x_data = (np.float64(node_list[i].parent_x)/l* 17) - 6   #對整個矩陣做運算need to be np
    y_data = 8 - np.float64(node_list[i].parent_y)/h* 12

    print(x_data.shape[0])
    real_data = np.empty((0,2), float)
    for num in range(x_data.shape[0]):
        real_data = np.append(real_data, [[x_data[num], y_data[num]]], 0)

    print(real_data)
    np.save("RRT_position", real_data)
    print("save path of the RRT\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', type=int, default=30,metavar='Stepsize', action='store', dest='stepSize',
    ...
    if args.selectPoint:
        ...
        target_location = {'refrigerator':[177, 205], 'rack':[283, 139], 'cushion':[400, 211], 'lamp':[357, 297], 'cooktop':[145, 253]} #圓括號是tuple不可更改 方括號是list做更改
        Input_target = input("Target name: ")
        print("Select start point by double clicking, press 'escape' to exit")
        ...
        end=(target_location[Input_target][0], target_location[Input_target][1])
